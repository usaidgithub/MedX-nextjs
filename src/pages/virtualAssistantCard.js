...
const VirtualAssistantCard = ({ isOpen, onClose, onVoiceSubmit, botReply, shouldSpeak, onSpoken }) => {
  ...
  const currentUtterance = useRef(null); // Ref to keep track of the current speech synthesis
  ...
  const speakText = (text) => {
    ...
    utterance.onend = () => {
      // Clean up the ref when speech finishes
  ...
  // Effect to handle speaking the bot's reply
  useEffect(() => {
    if (shouldSpeak && botReply) {
      speakText(botReply);
    }
  }, [botReply, shouldSpeak]);

  // Effect to handle cleanup on component unmount or when card closes
  useEffect(() => {
    ...
    return () => {
      ...
      // Clean up speech synthesis and speech recognition on unmount or close
  ...
  // Effect to manage speech recognition setup
  useEffect(() => {
  ...
  }, [onVoiceSubmit, isOpen]); // Added isOpen dependency

  // Effect to manage body overflow and stop listening/speaking when card closes
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = "hidden";
    } else {
      document.body.style.overflow = "auto";
      // Stop listening when the card closes
      stopListening();
      // Stop speaking when the card closes
      if (window.speechSynthesis && window.speechSynthesis.speaking) {
        window.speechSynthesis.cancel();
      }
    }
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md animate-fadeIn transition-opacity">
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-[90%] max-w-sm p-8 relative transform transition-all duration-300 scale-100">
            ...
            <button
              onClick={() => {
                onClose(); // Call the onClose prop first
                // Ensure speech is cancelled immediately when close button is clicked
                if (window.speechSynthesis && window.speechSynthesis.speaking) {
                  window.speechSynthesis.cancel();
                }
              }}
